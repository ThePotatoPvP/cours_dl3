...
\begin{exo}
    ...
    \begin{q}{1}
        ...
        \boxans{Notons \(B_{k,n}\) ce nombre, on remarque la formule de récurrence
        \(B_{k,n} = \sum_{i=0}^n B_{k-1,i}\). On remarque ensuite que pour tout \(k\in\N\)
        on a \(B_{k,0}=1\) et pour tout \(n\in\N,B_{1,n}=1\). On a aussi \(B_{k,1}=k\).
        La formule \(B_{k,n}=\binom{n+k-1}{n}\) semble vraie.}
    \end{q}
    \begin{q}{2}
        Même question pour \(\N^*\) et \(\Z\).
        \boxans{Dans \(\Z\) on a trivialement un nombre infini d'uplets pour \(k\geq2\)
        et sinon \(k\). Dans \(\N^*\) on modifie légèrement la formule de la
        question précédente pour avoir \(B_{k,n}=\sum_{i=0}^{n-1} B_{k,i}\) et
        les cas de base deviennent \(B_{k,0}=0\) et \(B_{1,n}=1\).}
    \end{q}
\end{exo}
...
\begin{exo}
    ...
    \boxans{Il y en a exactement que de sous parties de \(\llbracket1,n\rrbracket\)
    à au plus \(k\) éléments donc \(\sum_{i=0}^k \binom{n}{k}\)}
\end{exo}
...
\begin{exo}
    ...
    \boxans{On a la formule suivante \(a_n = a_{n-1} + a_{n-2}\).
    On retrouve \textsc{Fibonacci}.}
\end{exo}
