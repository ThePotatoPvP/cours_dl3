...
\begin{exo} Sur les sommes
    ...
    \begin{q}{2}
        ...
        \[\sum_{i=0}^ni^2=2*\sum\binom{i}{2}+\sum i = \binom{n+1}{3}+\frac{n(n+1)}{2}=\frac{n(n+1)(2n+1)}{6}\]
...
\begin{exo}
    ...
    \[\binom{n+1}{k_1,\dots,k_m}=\binom{n}{k_1-1,\dots,k_m}+\dots+\binom{n}{k_1,\dots,k_m-1}\]
    \boxans{Comme \(n+1\) est la somme des \(k_i\) on a que la partie de droite peut
    se réécrire comme \(\sum k_i \binom{n}{k_1,\dots,k_m}\) ce qui donne finalement
    le membre de gauche de l'équation, c'est bueno.}
\end{exo}
...
\begin{exo}
    ...
    d'au moins une tour.
    \boxans{On a \(n^n\) façons de mettre une tour par colonne, ce qui indirectement
    couvre les lignes, pareil par symétrie axiale, et on a alors \(n!\) cas qui se répètent,
    ceux où chaque ligne et chaque colonne ont une tour, finalement on a \(2n^n-n!\)
    façons de le faire, tah \textsc{Poincarré}.}
\end{exo}
...
\begin{exo}
    ...
    \begin{q}{1}
        ...
        points fixes.
        \boxans{On a la formule de récurrence \(d_n = n(d_{n-1}+d_{n-2})\) %ce qui livre
        %\(d_n = n! \sum\frac{(-1)^i}{i!}\) par récurrence immédiate.}
    ...
    \begin{q}{2}
        ...
        \boxans{On choisit les \(k\) points fixes et on permutte tous les autres éléments
        de sorte à ne pas avoir de points fixes parmi eux, on rajoute donc un facteur
        \(\binom{n}{k}\) devant. On veut alors permutter que \(n-k\) éléments.}
    \end{q}
\end{exo}
...
\begin{exo}
    ...
    \begin{q}{1}
        ...
        \boxans{Pour chacune des \(n\) boules on a \(k\) choix possibles donc \(T=n^k\).}
    \end{q}
    \begin{q}{2}
        ...
        et de \(|\bigcup M_i|\).
        \boxans{\(B = T - |\bigcup M_i|\) vu qu'on veut aucune boite vide, on veut tout
        sauf les cas avec au moins une boite vide.}
    \end{q}
    \begin{q}{3}
        En déduire une expression de \(B\) et de \(S(n,k)\).
        \boxans{Le cardinal de l'union des \(M_i\) s'obtient par la formule du crible
        de \textsc{Poincarré} : \(|\bigcup M_i | = \sum (-1)^{i+1} n^{k-i}\)}
    \end{q}
\end{exo}
\end{document}