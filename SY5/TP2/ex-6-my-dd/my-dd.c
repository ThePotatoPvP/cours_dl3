...
int main(int argc, char * argv[]) {
   ...
   * Ne pas oublier de tester s'il y a eu une erreur. */
  if (out_file != NULL){
    out_fd = open(out_file, O_RDWR | O_CREAT, S_IWUSR | S_IRUSR);
   ...
   * Ne pas oublier de tester s'il y a eu une erreur. */
  if (close(in_fd) == -1){return 1;}
  if (close(out_fd) == -1){return 1;}
...
int do_dd(int fd_in, int fd_out, size_t bs, int count) {
  ...
  for(int i=0; i<count; i++){
    result = read(fd_in, buffer, bs);
    if (result == -1){return 1;}
    if (result == 0){break;}

    if (write(fd_out, buffer, result) == -1){return 1;}
  }
  free(buffer);
  return 0;
}
